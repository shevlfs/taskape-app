...
struct GroupMembersView: View {
                    ...
                    List {
                        ForEach(group.users) { user in
                            HStack {
                                ...
                                if user.id != group.creatorId,
                                   user.id != UserManager.shared.currentUserId,
                                   group.creatorId == UserManager.shared.currentUserId
                                {
                                    Button {
                                        removeUser(user)
                                    } label: {
    ...
    private func loadMembers() {
        ...
        Task {
            await groupManager.loadGroupMembers(group: group, context: modelContext)
    ...
    private func removeUser(_ user: taskapeUser) {
        Task {
            ...
            if success {
                await MainActor.run {
                    if let index = group.members.firstIndex(of: user.id) {
                        group.members.remove(at: index)
                    }

                    group.users.removeAll { $0.id == user.id }
                }
            }
        }
    }
