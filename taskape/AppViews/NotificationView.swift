...
class NotificationStore: ObservableObject {
    ...
    func markAsRead(_ notification: NotificationModel) {
        if let index = notifications.firstIndex(where: {
            ...
            var updatedIds = readNotificationIds
            updatedIds.insert(notification.id)
            UserDefaults.standard.set(
                Array(updatedIds), forKey: readNotificationsKey
            )
    ...
    func refreshNotifications(modelContext: ModelContext) {
        isLoading = true
    ...
    private func refreshGroupInvites(notifications _: inout [NotificationModel])
        async
    {
        /*
         if let groupInvites = await getGroupInvites() {
             for invite in groupInvites {
                 notifications.append(NotificationModel(
                     ...
                     type: .groupInvite,
                     data: invite,
                     timestamp: ISO8601DateFormatter().date(from: invite.created_at) ?? Date()
                 ))
             }
         }
         */
